...
def trainModel(name, mode, train, val, n_users, n_times, n_apps, args, device):
    ...
    train_dataset = DNNDataset(train)   # (636139, 5)
    val_dataset = DNNDataset(val)       # (90877, 5)
    ...
    # model & optimizer
    model = DNN(n_users, n_times, n_apps, args.dim, args.hidden, args.seq_length)
    model.to(device)
...
def testModel(name, mode, test, n_users, n_times, n_apps, args, device):
    test_dataset = DNNDataset(test)   # (181754, 5)
...
def main():
    ...
    df_dnn = pd.read_csv('../data/baseline.txt', sep='\t')
    df_dnn['app_seq'] = df_dnn['app_seq'].apply(ast.literal_eval)
    df_dnn['recent_apps'] = df_dnn['recent_apps'].apply(ast.literal_eval)
    df_dnn['time'] = df_dnn['time'].apply(lambda x: str(x))
    # time的转换 [示例：0_13，周末为1，工作日为0；13表示13点]
    df_dnn['time'] = df_dnn['time'].apply(lambda x: prep_time(x))
    ...
    stratify_seed = df_dnn['user']
    df_dnn['user'] = user_encoder.transform(df_dnn['user'])
    df_dnn['time'] = time_encoder.transform(df_dnn['time'])
    df_dnn['app'] = app_encoder.transform(df_dnn['app'])
    df_dnn['app_seq'] = df_dnn['app_seq'].apply(lambda x: app_encoder.transform(x))
    ...
    trainval, test = train_test_split(df_dnn, test_size=0.2, random_state=2021, stratify=df_dnn['user'])
