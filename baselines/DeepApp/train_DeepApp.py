...
def evaluateModel(model, criterion, data_loader, device, args):
    ...
    with torch.no_grad():
        for user, time, loc, app, app_seq in data_loader:
            ...
            u_scores, l_scores, a_scores = model(time, loc, app_seq) # [batch_size, num_apps]
            l = computeLoss(u_scores, l_scores, a_scores, user, loc, target, device, args)
    ...
    with torch.no_grad():
        for user, time, loc, app, app_seq in data_loader:
            ...
            for idx, k in enumerate(Ks):
                correct = torch.sum(torch.eq(torch.topk(a_scores, dim=1, k=k).indices, target)).item()
                corrects[idx] += correct
    return corrects

def computeLoss(u_scores, l_scores, a_scores, user, loc, targets, device, args):
    # app loss
    preds = torch.topk(a_scores, dim=1, k=args.topk).indices     # [batch_size, k]
    ...
    loss_a = F.cross_entropy(a_scores, targets.view(-1), reduction='none')
    loss_a = torch.mean(torch.mul(coefficient, loss_a))

    # loc loss
    loss_l = F.cross_entropy(l_scores, loc.view(-1), reduction='none')
    loss_l = torch.mean(loss_l)
    # user loss
    loss_u = F.cross_entropy(u_scores, user.view(-1), reduction='none')
    loss_u = torch.mean(loss_u)

    loss = loss_a + args.lambda_u * loss_u + args.lambda_l * loss_l
...
def trainModel(name, mode, train, val, n_users, n_times, n_locs, n_apps, args, device):
    with open('Log/' + KEYWORD + '_' + name + '_log.txt', 'a') as f:
        ...
        f.write("topk: {}\n".format(args.topk))
        f.write("alpha: {}\n".format(args.alpha))
        f.write("lambda_u: {}\n".format(args.lambda_u))
        f.write("lambda_l: {}\n".format(args.lambda_l))

    train_dataset = DeepAppDataset(train)   # (636139, 5)
    val_dataset = DeepAppDataset(val)       # (90877, 5)
    ...
    # model & optimizer
    model = DeepApp(n_users, n_times, n_locs, n_apps, args.hidden, args.dim, args.seq_length, args.model_type)
    model.to(device)
    ...
    for epoch in range(args.epoch):
        ...
        for i, (user, time, loc, app, app_seq) in enumerate(train_loader):
            ...
            loss = computeLoss(u_scores, l_scores, a_scores, user, loc, target, device, args)
            loss.backward()
            optimizer.step()
...
def testModel(name, mode, test, n_users, n_times, n_locs, n_apps, args, device):
    test_dataset = DeepAppDataset(test)   # (181754, 5)
...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, default='GRU', help="model type: RNN/LSTM/GRU")
    ...
    parser.add_argument('--lambda_u', type=float, default=0.2, help="user for loss function")
    parser.add_argument('--lambda_l', type=float, default=0.2, help="location for loss function")
    return parser.parse_args()


MODELNAME = "DeepApp"
#KEYWORD = 'pred_Public' + '_' + MODELNAME + '_' + datetime.datetime.now().strftime("%y%m%d%H%M")
KEYWORD = 'pred_Public' + '_' + MODELNAME + '_2301281411'
