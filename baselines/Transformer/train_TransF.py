...
def evaluateModel(model, criterion, data_loader, device, args):
    ...
    l_sum, n = 0.0, 0
    with torch.no_grad():
        for user, time, app, app_seq in data_loader:
            user = user.to(device)
            time = time.to(device)
...
def trainModel(name, mode, train, val, n_users, n_times, n_apps, args, device):
    ...
    train_dataset = TransFDataset(train)   # (636139, 5)
    val_dataset = TransFDataset(val)       # (90877, 5)
    ...
    # model & optimizer
    model = TransFormer(n_users, n_times, n_apps, args.hidden, args.dim, args.seq_length)
    model.to(device)
...
def testModel(name, mode, test, n_users, n_times, n_apps, args, device):
    test_dataset = TransFDataset(test)   # (181754, 5)
...
def main():
    ...
    df_tf = pd.read_csv('../data/baseline.txt', sep='\t')
    df_tf['app_seq'] = df_tf['app_seq'].apply(ast.literal_eval)
    df_tf['recent_apps'] = df_tf['recent_apps'].apply(ast.literal_eval)
    df_tf['time'] = df_tf['time'].apply(lambda x: str(x))
    # time的转换 [示例：0_13，周末为1，工作日为0；13表示13点]
    df_tf['time'] = df_tf['time'].apply(lambda x: prep_time(x))
    ...
    # 特征编码
    user_encoder.fit(df_tf['user'].unique())
    time_encoder.fit(df_tf['time'].unique())
    all_apps = list(df_tf['app'].unique())
    for app_seq in df_tf['app_seq']:
        all_apps.extend(app_seq)
    app_encoder.fit(list(set(all_apps)))

    stratify_seed = df_tf['user']
    df_tf['user'] = user_encoder.transform(df_tf['user'])
    df_tf['time'] = time_encoder.transform(df_tf['time'])
    df_tf['app'] = app_encoder.transform(df_tf['app'])
    df_tf['app_seq'] = df_tf['app_seq'].apply(lambda x: app_encoder.transform(x))
    
    # 输入特征向量的维度
    num_users = len(df_tf['user'].unique())
    num_times = len(df_tf['time'].unique())
    num_apps = len(app_encoder.classes_)

    # split: trian/val/test
    trainval, test = train_test_split(df_tf, test_size=0.2, random_state=2021, stratify=df_tf['user'])
    ...
    trainModel(MODELNAME, 'train', train, val, num_users, num_times, num_apps, args, device)
    print(KEYWORD, 'testing started', clock.ctime())
    testModel(MODELNAME, 'test', test, num_users, num_times, num_apps, args, device)
